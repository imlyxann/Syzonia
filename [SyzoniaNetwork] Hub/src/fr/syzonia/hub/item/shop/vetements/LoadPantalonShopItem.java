...
public class LoadPantalonShopItem {

	public void load(VirtualMenu menu, Player player) {
		menu.setItem(0, new ItemBuilder().type(Material.LEATHER_LEGGINGS).setLeatherColor(Color.BLACK).name("§ePantalon Nike").lore("§8--------------------", "§ePantalon", "§6Tu as du Style? Oui!", "§8--------------------", "", (VetementsManager.havePantalon(1, player) ? "Vous possédez cet objet" : "Achetez cet objet "  + new PriceUtils().Price(200) + " Syzocoins " + new PriceUtils().getPromo(200))).setSkullOwner(player.getName()).build());
	}
