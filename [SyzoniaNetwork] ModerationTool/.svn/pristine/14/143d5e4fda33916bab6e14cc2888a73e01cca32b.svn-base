	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(!(sender instanceof Player)) {
			...
			if(PlayerInfo.getInfos(player.getUniqueId()).getRank() >= 5) {
				if(args.length != 1) {
					player.sendMessage("§cIl faut faire la commande: /unfreeze <player>");
					return false;
				}
				...
				if(Main.PlayerFreeze.contains(targetName)) {
					Main.PlayerFreeze.remove(targetName);
					new Title().sendFullTitle(targetPlayer, 10, 30, 10, "§bUnFreeze", "§aTu peux sortir!");
					targetPlayer.sendMessage("§7[§bFreeze§7] §4Tu as été UnFreeze par §c" + player.getDisplayName());
				} else {
					player.sendMessage("§cCe joueur n'est pas freeze");
				}
			}
		}
